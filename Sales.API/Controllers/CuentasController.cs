...
namespace Sales.API.Controllers
{
    ...
    public class CuentasController : ControllerBase
    {
        ...
        {
            string path = Path.Combine(AppContext.BaseDirectory, "Scripts", "MigracionInicial.sql");
        ...
        {
            var queryable = context.Cuentas
                .Where(x => x.EmpresaId == pagination.EmpresaId)
                .AsQueryable();

            if (!string.IsNullOrWhiteSpace(pagination.Filter))
            {
                queryable = queryable.Where(x => x.CodigoCuenta.ToLower().Contains(pagination.Filter.ToLower()));
            }
        ...
        [HttpGet("totalPages")]
        public async Task<ActionResult> GetPages([FromQuery] PaginationDTO pagination, int empresaId)
        {
            var queryable = context.Countries.AsQueryable();
